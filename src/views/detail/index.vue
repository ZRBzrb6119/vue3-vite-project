    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImageList="skuImageList" :currentIndex="currentIndex"/>
          <!-- 小图列表 -->
          <ImageList :skuImageList="skuImageList" @changeindex="changeindex"/>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
...
<script setup>
import ImageList from "./detailChild/ImageList.vue"
import Zoom from "./detailChild/Zoom.vue"
...
// let {categoryView,skuInfo,spuSaleAttrList} = store.state.detail.goodInfo
...
function changeSkuNum(event){
  ...
  else skuNum.value=parseInt(value)
}

async function addShopcar(){
  try{
    let res=await reqShopCart(route.params.skuid,skuNum.value)
    if(res.code===200){
